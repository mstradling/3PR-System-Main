...
def enter_3pr_mode():
        ...
        while True:
                ...
                #       print("speed")
                #try:
                report = session.next()         # get data from gps
                ...
                if report['class'] == 'TPV':
                        ...
                        #       print report.time
                        if (count == 0):
                                headingInit = report.heading
                        count = 1
                        #else if hasattr(report, 'heading'):
                        #       print report.heading
         #              if hasattr(report, 'speed'):
        #               print report.speed * gps.MPS_TO_KPH

                        heading = report.heading - headingInit          # calculations
                        x = np.sin(heading)*report.speed*MPH_TO_MPS
                        y = np.cos(heading)*report.speed*MPH_TO_MPS

                        process_stdin(("setyaw %f 1 1" %heading))       # instructions
                        process_stdin(("velocity %f %f 1", x,y))
                        #return
                #except KeyError:
                #       pass
...
def cmd_hello():
        print("Greetings all-powerful programmer, how can I please you?")
        #mpstate.mav_param.mavset(mpstate.master(), setyaw, 10, retries=3)
        #myYaw = 10
        #process_stdin(("setyaw %d 1 1" %myYaw))
        return
...
if __name__ == '__main__':
    ...
    parser.add_option("--default-modules", default="log,signing,wp,rally,fence,param,relay,tuneopt,arm,mode,calibration,r$
    ...
    if not opts.master:
        serial_list = mavutil.auto_detect_serial(preferred_list=['*FTDI*',"*Arduino_Mega_2560*", "*3D_Robotics*", "*USB_t$
